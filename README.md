...
- [x] 场景搜索入口、连续路径导航与可直达的概念对比
- [x] 统一核验状态展示及运行时、独立校验器的内容约束
- [x] 九篇核心深读内容、逐项来源说明与可展开自测
- [x] 首个无需 API 密钥的可执行校验示例，并直接测试正文代码
- [ ] 完善重点决策案例与专题证据档案
- [ ] 扩展到 40 个高价值 AI 应用工程词条
- [ ] 扩充检索、工具调用与评测的可复现代码示例
- [ ] 自动检查外部来源可访问性与核验日期
- [ ] 基于同一内容源提供独立 MCP Server

版本变化记录在 [CHANGELOG.md](CHANGELOG.md)。路线图会根据真实使用反馈调整，不承诺固定发布日期。

产品流程、内容核验责任和验收清单见 [产品行为约定](docs/PRODUCT_BEHAVIOR.md)。路径、对比和雷达建议集中在 `lib/editorial.ts`；词条 Schema 集中在 `lib/term-schema.ts`，供应用与独立校验器共同使用。

本轮九篇内容的核验范围、证据边界与新增阅读字段见 [核心内容核验记录](docs/CONTENT_REVIEW.md)。其余词条尚未完成同等深度扩写。
